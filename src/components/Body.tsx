...
export default function Body() {
    ...
    return (
        <main className="grid grid-cols-1 gap-20 w-full md:gap-50">
            <section className="flex flex-col gap-5 items-center md:items-start w-full md:relative md:max-w-[800px] md:place-self-center">
                <img src={interactiveImg} alt="A man in a blue t-shirt wearing a VR-headset and is in a T-Rex like posture with his arms." className="md:h-75 md:w-100" />

                <div className="flex flex-col md:text-left md:gap-5 md:w-90 md:absolute md:top-30 md:right-30 md:p-10 bg-white">
                ...
                <div className="grid grid-rows-8 gap-5 w-full md:grid-rows-2 md:grid-cols-4">
                {
                    ...
                    return (
                        <div>
                            ...
                            <section key={index} 
                                     className={`[ text-white text-xl text-bottom font-light ] [ transition delay-150 duration-300 ease-in-out hover:scale-105 active:scale-105 hover:cursor-pointer ] [ ${bgImageDesktop[count]} bg-center bg-contain bg-no-repeat ] [ p-5 ] [ min-h-80 ] [ relative ] max-md:hidden max-md:aria-hidden`}>
                                <h3 className="absolute bottom-5 select-none">{item}</h3>
                            </section>
                        </div>   
                    )
